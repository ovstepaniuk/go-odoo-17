...
func (c *Client) CreateAccountTourUploadBillEmailConfirm(atubec *AccountTourUploadBillEmailConfirm) (int64, error) {
	ids, err := c.CreateAccountTourUploadBillEmailConfirms([]*AccountTourUploadBillEmailConfirm{atubec})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountTourUploadBillEmailConfirm creates a new account.tour.upload.bill.email.confirm model and returns its id.
func (c *Client) CreateAccountTourUploadBillEmailConfirms(atubecs []*AccountTourUploadBillEmailConfirm) ([]int64, error) {
	var vv []interface{}
	for _, v := range atubecs {
		vv = append(vv, v)
	}
	return c.Create(AccountTourUploadBillEmailConfirmModel, vv, nil)
}
...
func (c *Client) UpdateAccountTourUploadBillEmailConfirms(ids []int64, atubec *AccountTourUploadBillEmailConfirm) error {
	return c.Update(AccountTourUploadBillEmailConfirmModel, ids, atubec, nil)
}
...
func (c *Client) FindAccountTourUploadBillEmailConfirmIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(AccountTourUploadBillEmailConfirmModel, criteria, options)
}
