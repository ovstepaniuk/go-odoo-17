...
func (c *Client) CreateBaseDocumentLayout(bdl *BaseDocumentLayout) (int64, error) {
	ids, err := c.CreateBaseDocumentLayouts([]*BaseDocumentLayout{bdl})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateBaseDocumentLayout creates a new base.document.layout model and returns its id.
func (c *Client) CreateBaseDocumentLayouts(bdls []*BaseDocumentLayout) ([]int64, error) {
	var vv []interface{}
	for _, v := range bdls {
		vv = append(vv, v)
	}
	return c.Create(BaseDocumentLayoutModel, vv, nil)
}
...
func (c *Client) UpdateBaseDocumentLayouts(ids []int64, bdl *BaseDocumentLayout) error {
	return c.Update(BaseDocumentLayoutModel, ids, bdl, nil)
}
...
func (c *Client) FindBaseDocumentLayoutIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(BaseDocumentLayoutModel, criteria, options)
}
